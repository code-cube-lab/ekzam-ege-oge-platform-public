...
const answerKey = [1, 2, 0, 1, 2, 0, 1, 2, 1, 1];
const topics = [
  "Орфоэпия",
  ...
  "Смысловая связь примера и тезиса",
];
...
export async function POST(request: Request) {
  ...
  if (!Array.isArray(body.answers) || body.answers.length !== answerKey.length) {
    return Response.json({ error: "Ten answers required" }, { status: 400 });
  }
  const correct = answerKey.reduce(
    (total, value, index) => total + (body.answers?.[index] === value ? 1 : 0),
    0,
  );
  const score = Math.round(40 + correct * 6);
