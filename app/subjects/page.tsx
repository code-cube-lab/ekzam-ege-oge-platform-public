...
export default function SubjectsPage() {
  return <main className="catalog-page">
    <header className="faculty-top">
      ...
      <div className="catalog-actions"><Link href="/exam">ОГЭ / ЕГЭ</Link><Link href="/teachers">Преподаватели</Link><Link className="button button-red button-small" href="/exam">Начать практику</Link></div>
    </header>
    <section className="catalog-hero">
      <div><span className="exam-label">15 предметов ЕГЭ · 14 предметов ОГЭ</span><h1>Выберите предмет.<br /><em>Решайте в формате экзамена.</em></h1></div>
    ...
    <section className="subject-lead-grid" aria-label="Предметы и преподаватели">
      {examSubjects.map((subject, index) => {
        ...
        return <article className="subject-lead-card" key={subject.slug}>
          ...
          <h2>{subject.name}</h2>
          {lead ? <><div className="lead-person"><TeacherPhoto lead={lead} /><div><b>{lead.teacher}</b><small>{lead.department}</small></div></div><p>{subject.slug === "russian" ? "Отдельные варианты ОГЭ и ЕГЭ" : "Стартовый авторский банк"}: {subject.focus.join(", ")}. Задания решаются по одному прямо на платформе.</p><div className="lead-foot"><span>Участие уточняется</span><Link href={`/exam?subject=${subject.slug}`}>Открыть предмет →</Link></div></> : <><div className="lead-person"><span className="teacher-photo-fallback">+</span><div><b>Преподаватель подбирается</b><small>Иностранный язык</small></div></div><p>Подключены экзаменационные умения и стартовые задания на сайте. Платная группа пока не открыта.</p><div className="lead-foot"><span>Набор не открыт</span><Link href={`/exam?subject=${subject.slug}`}>Открыть предмет →</Link></div></>}
        </article>;
      })}
    </section>
