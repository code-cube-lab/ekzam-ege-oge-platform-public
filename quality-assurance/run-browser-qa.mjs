...
try {
  ...
  assert(await page.locator(".teacher-cabinet-sidebar nav button").count() === 6, "персональный кабинет содержит шесть самостоятельных страниц");
  ...
  assert(await mobilePage.locator(".teacher-cabinet-mobile-nav button").count() === 6, "мобильная версия сохраняет шесть страниц кабинета");
